...
class Agency(Core_UI):
        ...
        with dpg.tab(label="Agency", parent=parent, before=befor) as backTab:
                ...
                with dpg.group(horizontal=True):
                    dpg.add_button(
                        ...
                        callback=lambda: dpg.configure_item(
                            self.saveFileDialog, show=True
                        ),
                    )
                    dpg.add_button(
                        label="Load",
                        callback=lambda: dpg.configure_item(
                            self.loadFileDialog, show=True
                        ),
                    )
            with dpg.group(horizontal=True):
                with dpg.child_window(width=200, height=150) as self.primaryAgencycw:
                    dpg.add_image(self.agencyLogo, width=100, height=100)
                    dpg.add_text(self.agency.company_name)
                with dpg.child_window(width=600, height=150):
                ...
                with dpg.child_window(
                    ...
                    self.clientDisplay: int | str = dpg.add_text("Selected Client")
            with dpg.child_window(width=-1, height=-1):
                with dpg.group(horizontal=True):
                    ...
                    with dpg.child_window(width=400, height=-1):
                        self.reportTitle = dpg.add_input_text(
                            label="Report Title", default_value="SEO Report"
                        )
                        self.reportSubtitle = dpg.add_input_text(
                            default_value="SEO Report Subtitle"
                        )
                        self.reportFooterLeft = dpg.add_input_text(
                            default_value="Report for CLIENT"
                        )
                        self.reportFooterCenter = dpg.add_input_text(
                            default_value="Your Key to Success"
                        )
                        self.reportFooterRight = dpg.add_input_text(
                        ...
                        self.reportCovercw = dpg.add_child_window(width=375, height=-1)
                        self.reportCover = ""
                    with dpg.group():
    ...
    def generateReport(self) -> None:
        client: str = dpg.get_value(self.clientDisplay)
        if client not in self.clientList:
            return
        reportTitle: str = dpg.get_value(self.reportTitle)
        reportSubtitle: str = dpg.get_value(self.reportSubtitle)
        reportFooterLeft: str = dpg.get_value(self.reportFooterLeft)
        reportFooterCenter: str = dpg.get_value(self.reportFooterCenter)
        reportFooterRight: str = dpg.get_value(self.reportFooterRight)
        reportCover: str = self.reportCover
        ...
        proxy: str = dpg.get_value(self.proxyBox)
    ...
    def loadFile(self, sender, app_data, user_data, *args, **kwargs) -> None:
        ...
        if user_data == "client":
            ...
            dpg.add_image(
                self.tempCover, parent=self.reportCovercw, width=200, height=350
            )
    ...
    def uploadReportCover(self, *args, **kwargs) -> None:
        ...
        dpg.delete_item(self.reportCovercw, children_only=True)
        dpg.add_image(self.tempCover, parent=self.reportCovercw, width=200, height=350)
        # dpg.configure_item(self.loadFileDialog, show=True)
